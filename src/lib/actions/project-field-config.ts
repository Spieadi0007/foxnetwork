...
export async function getProjectFieldDefinitions(): Promise<{
  data: ProjectFieldDefinition[] | null
  error: string | null
}> {
  const supabase = await createClient()
...
export async function getProjectFieldConfigs(): Promise<{
  data: ProjectFieldConfig[] | null
  error: string | null
}> {
  const supabase = await createClient()
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      field_definition:project_field_definitions(*)
    `)
    .eq('company_id', companyId)

  if (error) {
    console.error('Error fetching project field configs:', error)
    return { data: null, error: error.message }
  }
...
export async function upsertProjectFieldConfig(
  input: UpdateProjectFieldConfigInput
): Promise<{ data: ProjectFieldConfig | null; error: string | null }> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
  ...
  if (error) {
    console.error('Error upserting project field config:', error)
    return { data: null, error: error.message }
  }
...
export async function bulkUpdateProjectFieldConfigs(
  configs: UpdateProjectFieldConfigInput[]
  ...
  if (error) {
    console.error('Error bulk updating project field configs:', error)
...
export async function getProjectCustomFields(): Promise<{
  data: ProjectCustomField[] | null
  error: string | null
}> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
...
): Promise<{ data: ProjectCustomField | null; error: string | null }> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
...
export async function updateProjectCustomField(
  id: string,
  input: Partial<CreateProjectCustomFieldInput>
): Promise<{ data: ProjectCustomField | null; error: string | null }> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
...
export async function deleteProjectCustomField(id: string): Promise<{ error: string | null }> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
...
export async function getResolvedProjectFields(): Promise<{
  data: ResolvedProjectField[] | null
  error: string | null
}> {
  const supabase = await createClient()
  const companyId = await getCompanyId()
  ...
  if (defError) {
    console.error('Error fetching project field definitions:', defError)
  ...
  if (configError) {
    console.error('Error fetching project field configs:', configError)
  ...
  if (customError) {
    console.error('Error fetching project custom fields:', customError)
  ...
  const resolvedFields: ResolvedProjectField[] = (definitions || []).map((def) => {
  ...
  // Add custom fields
  const customResolvedFields: ResolvedProjectField[] = (customFields || []).map((field) => ({
    key: field.field_key,
